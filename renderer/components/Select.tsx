...
const calculateSizeAndPosition = (optionsLength: number, buttonRef: MutableRefObject<HTMLButtonElement>) => {
...
const Select = (props: selectProps) => {
	...
	const options = useMemo(
		() => props.options.filter(option => !props.value?.find(value => value.value === option.value)),
		[props.options, props.value]
	);

	const { width, height, top, center } = calculateSizeAndPosition(props.options?.length || 0, buttonRef);
